...
local update = redis:get(TH3JOKER..":VERSION")
...
sendDocument(msg.to.id, msg.id_,0, 1, nil, "./plugins/"..string.match (v, "(.*)%.lua") ..".lua", '🗂¦ آلمـلف مـقدم مـن قنآ‏‏هہ آلجوكر ¦ֆ \n📡¦ تابع قناة السورس @llJOKERll\n👨🏽‍🔧', dl_cb, nil) end else

if not plugin_exists(matches[2]) then 

return '🗂*¦* لا يوجد ملف بهذا الاسم .\n\n'

else sendDocument(msg.to.id, msg.id_,0,1,nil,"./plugins/"..matches[2]..".lua",'🗂¦ آلمـلف مـقدم مـن قنآ‏‏هہ آلجوكر ¦ֆ \n📡¦ تابع قناة السورس @llJOKERll\n👨🏽‍🔧', dl_cb, nil)
...
redis:srem(TH3JOKER..':PLUGINS_FILE:',matches[2])
...
return"👨🏾‍🔧*¦* انتهى دعم هذا السورس بالتحديثات في قناة الجوكر @llJOKERll. \n📦"
