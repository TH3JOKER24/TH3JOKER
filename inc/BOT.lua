...
local url , res = https.request('https://lo_rd.tk/GetID/?User='..SUDO_USER)
if res ~= 200 then
...
local getversion = https.request('https://api.TH3JOKER.com/version/')
...
#!/usr/bin/env bash
cd $(cd $(dirname $0); pwd)
if ! screen -list | grep -q "TH3JOKER"; then
screen -S TH3JOKER lua ./inc/BOT.lua
fi
screen -X -S TH3JOKER kill
screen -S TH3JOKER lua ./inc/BOT.lua
]])
file:close()
...
]]..'\27[0;31m'..[[¦ Dev : @LO_RD]]..'\27[m\27[0;32m\n\n\027[0;32m'   
...
_info = load_config()
TH3JOKER = _info.TOKEN:match("(%d+)")
our_id = tonumber(TH3JOKER)
bot_username = redis:get(TH3JOKER..":username")
...
print('\n\27[0;32mPlugins is : '..#list_plug..' Are Active.\n\n\27[0;31mThe SOURCE RUN IS TH3JOKER v'..redis:get(TH3JOKER..':VERSION')..'...\n\27[m')
end
load_plugins()
...
return false end 
if not (tostring(msg.chat_id_):match('^-100') or tostring(msg.chat_id_):match('-')) and redis:get(TH3JOKER..':mute_pv:'..msg.sender_user_id_) then
...
if redis:get(TH3JOKER..'mute_text'..msg.chat_id_) then --قفل الدردشه
